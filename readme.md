...
Every figure below is regenerated by a cold-cache run and traces to a file in
[`outputs/`](outputs/).

| | Local `llama3.1:8b` | Gemini |
|---|---|---|
| **Invented citation ids** | **0** | **0** |
| **Refusals carrying citations** | **0** | **0** |
| **Abstention accuracy** on unanswerable controls | **4/4** | **4/4** |
| **Condensation drift rate** | **0/13** | **0/13** |
...
| p50 turn latency | **15.0 s** | 17.4 s |

| | |
|---|---|
| Recall@5 — hybrid + rerank *(LLM-free, identical on both)* | **0.646**, up from 0.479 single-retriever |
| Rate-limit rejections across 607 billed calls | **0** |
| Daily answer capacity, 4 keys | 4,480 API calls, then an **unlimited local floor** |
| Runs with no API key of any kind | **all of them** |
| Tests | 171, no network, no model downloads |
| Design decisions logged | 87, each with evidence |

**Zero invented citations is structural, not luck.** A citation is validated against
the chunk ids actually in context before the answer is rendered, so a fabricated id
cannot reach the reader — and a refusal has nothing to cite by construction.
...
**Single-turn route accuracy of 1.000 has reproduced on four independent runs**, as
have 4/4 abstention, zero invented citations and zero condensation drift. Only the
current run is tabulated: the earlier three predate the routing change, and quoting
them beside current numbers would be exactly the stale-figure problem this build now
guards against.
...
**The local path has the tighter latency distribution** — p95 is 1.2× its p50, against
2.3× for the hosted one. A local model has no queue and no quota, so it has no tail.
When a hosted rung does slow down or refuse, the ladder steps down and the run
completes: **zero rate-limit rejections across 607 billed calls** ([§9](#9--quota-engineering)).
